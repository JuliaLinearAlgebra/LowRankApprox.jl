      ...
      for T in (Float32, Float64, ComplexF32, ComplexF64)
        let A
            ...
            for (N, ρ, q) in ((M, m, n), (Mh, n, n), (Ms, n, n))
              ...
              y = At_mul_B(A,xp); @test norm(y - At_mul_B(F,xp)) < approx_rtol*norm(y)

              Bp = rand(T, ρ, ρ)
              Bq = rand(T, q, q)
              C = A   *Bq  ; @test norm(C - F   *Bq  ) < approx_rtol*norm(C)
              C = A   *Bq' ; @test norm(C - F   *Bq' ) < approx_rtol*norm(C)
              C = A_mul_Bt(A, Bq); @test norm(C - A_mul_Bt(F, Bq)) < approx_rtol*norm(C)
              C = A'  *Bp  ; @test norm(C - F'  *Bp  ) < approx_rtol*norm(C)
              C = A'  *Bp' ; @test norm(C - F'  *Bp' ) < approx_rtol*norm(C)
              C = At_mul_B(A, Bp)  ; @test norm(C - At_mul_B(F, Bp)  ) < approx_rtol*norm(C)
              C = At_mul_Bt(A, Bp); @test norm(C - At_mul_Bt(F, Bp)) < approx_rtol*norm(C)
              C = Bp  *A   ; @test norm(C - Bp  *F   ) < approx_rtol*norm(C)
              C = Bq  *A'  ; @test norm(C - Bq  *F'  ) < approx_rtol*norm(C)
              C = A_mul_Bt(Bq, A) ; @test norm(C - A_mul_Bt(Bq, F) ) < approx_rtol*norm(C)
              C = Bp' *A   ; @test norm(C - Bp' *F   ) < approx_rtol*norm(C)
              C = Bq' *A'  ; @test norm(C - Bq' *F'  ) < approx_rtol*norm(C)
              C = At_mul_B(Bp, A)   ; @test norm(C - At_mul_B(Bp, F)   ) < approx_rtol*norm(C)
              C = At_mul_Bt(Bq, A) ; @test norm(C - At_mul_Bt(Bq, F) ) < approx_rtol*norm(C)
          end
        end
      end
    end
