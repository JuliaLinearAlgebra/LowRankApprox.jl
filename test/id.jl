      ...
      for T in (Float32, Float64, ComplexF32, ComplexF64)
        let A
            ...
            for (N, G, ρ, q) in ((full(V), V, V[:k], n), (A, F, m, n))
              ...
              y = At_mul_B(N, xp); @test norm(y - At_mul_B(G, xp)) < approx_rtol*norm(y)

              Bp = rand(T, ρ, ρ)
              Bq = rand(T, q, q)
              C = N   *Bq  ; @test norm(C - G   *Bq  ) < approx_rtol*norm(C)
              C = N   *Bq' ; @test norm(C - G   *Bq' ) < approx_rtol*norm(C)
              C = A_mul_Bt(N, Bq); @test norm(C - A_mul_Bt(G, Bq)) < approx_rtol*norm(C)
              C = N'  *Bp  ; @test norm(C - G'  *Bp  ) < approx_rtol*norm(C)
              C = N'  *Bp' ; @test norm(C - G'  *Bp' ) < approx_rtol*norm(C)
              C = At_mul_B(N, Bp)  ; @test norm(C - At_mul_B(G, Bp)  ) < approx_rtol*norm(C)
              C = At_mul_Bt(N, Bp); @test norm(C - At_mul_Bt(G, Bp)) < approx_rtol*norm(C)
              C = Bp  *N   ; @test norm(C - Bp  *G   ) < approx_rtol*norm(C)
              C = Bq  *N'  ; @test norm(C - Bq  *G'  ) < approx_rtol*norm(C)
              C = A_mul_Bt(Bq, N) ; @test norm(C - A_mul_Bt(Bq, G) ) < approx_rtol*norm(C)
              C = Bp' *N   ; @test norm(C - Bp' *G   ) < approx_rtol*norm(C)
              C = Bq' *N'  ; @test norm(C - Bq' *G'  ) < approx_rtol*norm(C)
              C = At_mul_B(Bp, N)   ; @test norm(C - At_mul_B(Bp, G)   ) < approx_rtol*norm(C)
              C = At_mul_Bt(Bq, N) ; @test norm(C - At_mul_Bt(Bq, G) ) < approx_rtol*norm(C)
            end
        end
      end
    end
