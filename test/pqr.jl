    ...
    @time for (t, s) in ((:none,                 :none ),
      ...
      for T in (Float32, Float64, ComplexF32, ComplexF64)
        let A
            ...
            y = At_mul_B(A, xm); @test norm(y - At_mul_B(F, xm)) < approx_rtol*norm(y)

            Bm = rand(T, m, m)
            Bn = rand(T, n, n)
            C = A   *Bn  ; @test norm(C - F   *Bn  ) < approx_rtol*norm(C)
            C = A   *Bn' ; @test norm(C - F   *Bn' ) < approx_rtol*norm(C)
            C = A_mul_Bt(A, Bn); @test norm(C - A_mul_Bt(F, Bn)) < approx_rtol*norm(C)
            C = A'  *Bm  ; @test norm(C - F'  *Bm  ) < approx_rtol*norm(C)
            C = A'  *Bm' ; @test norm(C - F'  *Bm' ) < approx_rtol*norm(C)
            C = At_mul_B(A, Bm)  ; @test norm(C - At_mul_B(F, Bm)  ) < approx_rtol*norm(C)
            C = At_mul_Bt(A, Bm); @test norm(C - At_mul_Bt(F, Bm)) < approx_rtol*norm(C)
            C = Bm  *A   ; @test norm(C - Bm  *F   ) < approx_rtol*norm(C)
            C = Bn  *A'  ; @test norm(C - Bn  *F'  ) < approx_rtol*norm(C)
            C = A_mul_Bt(Bn, A) ; @test norm(C - A_mul_Bt(Bn, F) ) < approx_rtol*norm(C)
            C = Bm' *A   ; @test norm(C - Bm' *F   ) < approx_rtol*norm(C)
            C = Bn' *A'  ; @test norm(C - Bn' *F'  ) < approx_rtol*norm(C)
            C = At_mul_B(Bm, A)   ; @test norm(C - At_mul_B(Bm, F)   ) < approx_rtol*norm(C)
            C = At_mul_Bt(Bn, A) ; @test norm(C - At_mul_Bt(Bn, F) ) < approx_rtol*norm(C)
